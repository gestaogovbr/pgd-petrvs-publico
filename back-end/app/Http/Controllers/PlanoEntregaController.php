...
class PlanoEntregaController extends ControllerBase
{
    ...
    public function checkPermissions($action, $request, $service, $unidade, $usuario)
    {
        switch ($action) {
            case 'QUERY':
                if (!$usuario->hasPermissionTo('MOD_PENT')) throw new ServerException("CapacidadeSearchText", "O usuário logado não tem permissão para consultar planos de entregas (MOD_PENT). [RN_PENT_V]");
                /*                 
                    (RN_PENT_V) CONSULTAR
            ...
            case 'STORE':           //incluir ou alterar um Plano de Entregas
                ...
                switch ($acao) {
                    case 'EDIT':    // alteração de um Plano de Entregas
                        if (!$usuario->hasPermissionTo('MOD_PENT_EDT')) throw new ServerException("ValidateUsuario", "O usuário logado não tem permissão para alterar planos de trabalho (MOD_PENT_EDT). [RN_PENT_L]");
                        break;
                    case 'INSERT':  // inclusão de um novo Plano de Entregas
                        if (!$usuario->hasPermissionTo('MOD_PENT_INCL')) throw new ServerException("ValidateUsuario", "O usuário logado não tem permissão para incluir planos de trabalho (MOD_PENT_INCL). [RN_PENT_Z]");
                        break;
                }
                break;
            case 'DESTROY':
                ...
                if (!$usuario->hasPermissionTo('MOD_PENT_EXCL')) throw new ServerException("CapacidadeDestroy", "O usuário logado não tem permissão para excluir planos de entregas (MOD_PENT_EXCL). [RN_PENT_X]");
                ...
                if (!$condition1) throw new ServerException("ValidatePlanoEntrega", "O plano de entregas não pode ser excluído porque não se encontra no status INCLUIDO ou AGUARDANDO HOMOLOGAÇÃO. [RN_PENT_X]");
                if (!$condition2) throw new ServerException("ValidateUsuario", "O plano de entregas não pode ser excluído porque o usuário logado não é lotado nem é um dos gestores da sua unidade executora. [RN_PENT_X]");
                /*                 
                    (RN_PENT_X) EXCLUIR
            ...
            case 'ARQUIVAR':        // ou DESARQUIVAR
                ...
                $condicoes = $service->buscaCondicoes(['id' => $data['id']]);
                if ($data['arquivar']) {
                    if (!($condicoes['planoConcluido'] || $condicoes['planoAvaliado'])) throw new ServerException("ValidatePlanoEntrega", "O plano não pode ser arquivado porque não se encontra no status CONCLUIDO ou AVALIADO. [RN_PENT_N]");
                    if ($condicoes['planoArquivado']) throw new ServerException("ValidatePlanoEntrega", "O plano não pode ser arquivado porque já se encontra arquivado. [RN_PENT_N]");
                    if (!($condicoes['gestorUnidadePlano'] || ($condicoes['unidadePlanoEhLotacao'] && $usuario->hasPermissionTo("MOD_PENT_ARQ")))) throw new ServerException("ValidateUsuario", "O plano de entregas não pode ser arquivado porque nenhuma das condições abaixo é atendida:\n" .
                        ...
                        "2. o usuário logado precisa ser lotado na unidade executora do plano e possuir a capacidade MOD_PENT_ARQ. [RN_PENT_N]");
                    /*                 
                        (RN_PENT_N) ARQUIVAR
                    ...
                    */
                    ...
                    if (!$condicoes['planoArquivado']) throw new ServerException("ValidatePlanoEntrega", "O plano não pode ser desarquivado porque não se encontra arquivado. [RN_PENT_W]");
                    if (!($condicoes['gestorUnidadePlano'] || ($condicoes['unidadePlanoEhLotacao'] && $usuario->hasPermissionTo("MOD_PENT_ARQ")))) throw new ServerException("ValidateUsuario", "O plano de entregas não pode ser desarquivado porque nenhuma das condições abaixo é atendida:\n" .
                        "1. o usuário logado precisa ser um dos gestores da unidade executora do plano; ou\n" .
                        "2. o usuário logado precisa ser lotado na unidade executora do plano e possuir a capacidade MOD_PENT_ARQ. [RN_PENT_W]");
                    /*
            ...
            case 'AVALIAR':
                ...
                if (!$condicoes['planoConcluido']) throw new ServerException("ValidatePlanoEntrega", "O plano de entregas não pode ser avaliado porque não se encontra no status CONCLUIDO. [RN_PENT_O]");
            ...
            case 'CANCELAR_PLANO':
                ...
                if (!$condition1) throw new ServerException("ValidatePlanoEntrega", "O plano de entregas não pode ser cancelado porque não se encontra em nenhum dos seguintes status: INCLUIDO, AGUARDANDO HOMOLOGAÇÃO, ATIVO ou CONCLUIDO. [RN_PENT_P]");
                if (!($condition2 || $condition3)) throw new ServerException("ValidateUsuario", "O plano de entregas não pode ser cancelado porque o usuário logado não é lotado nem é um dos gestores da sua unidade executora. [RN_PENT_P]");
                /*
            ...
            case 'CANCELAR_AVALIACAO':
                ...
                $condition3 = UsuarioService::isIntegrante('AVALIADOR_PLANO_ENTREGA', $data['entity']['unidade_id']);
                if (!$condicoes['planoAvaliado']) throw new ServerException("ValidatePlanoEntrega", "Não é possível cancelar avaliação de um plano de entregas que não se encontra avaliado. [RN_PENT_R]");
            ...
            case 'CANCELAR_CONCLUSAO':
                ...
                if (!$condition1) throw new ServerException("ValidatePlanoEntrega", "Não é possível cancelar a conclusão de um plano de entregas que não se encontra no status CONCLUIDO. [RN_PENT_S]");
            ...
            case 'CANCELAR_HOMOLOGACAO':
                ...
                $condition4 = UsuarioService::isIntegrante('HOMOLOGADOR_PLANO_ENTREGA', $data['entity']['unidade']['unidade_pai_id']);
                if (!$condition1) throw new ServerException("ValidatePlanoEntrega", "A homologação do plano de entregas não pode ser cancelada porque o plano não se encontra no status ATIVO. [RN_PENT_T]");
            ...
            case 'HOMOLOGAR':
                ...
                $condition4 = UsuarioService::isIntegrante('HOMOLOGADOR_PLANO_ENTREGA', $data['entity']['unidade']['unidade_pai_id']);
                $condition5 = !!Unidade::find($data['entity']['unidade_id'])->instituidora;
                if (!$condition1) throw new ServerException("ValidatePlanoEntrega", "O plano de entregas não pode ser homologado porque não se encontra no status AGUARDANDO HOMOLOGAÇÃO. [RN_PENT_Y]");
                if ($condition5) throw new ServerException("ValidatePlanoEntrega", "O plano de entregas não precisa ser homologado porque pertence a uma unidade instituidora. [RN_PENT_Y]");
            ...
            case 'LIBERAR_HOMOLOGACAO':
                ...
                if (!$condition1) throw new ServerException("ValidatePlanoEntrega", "O plano de entregas não pode ser liberado para homologação porque não se encontra no status INCLUIDO. [RN_PENT_AA]");
                if (!$condition2) throw new ServerException("ValidatePlanoEntrega", "O plano de entregas não pode ser liberado para homologação porque ainda não possui nenhuma entrega. [RN_PENT_AA]");
            ...
            case 'REATIVAR':
                ...
                if (!$condition1) throw new ServerException("ValidatePlanoEntrega", "O plano de entregas não pode ser reativado porque não se encontra no status SUSPENSO. [RN_PENT_AC]");
            ...
            case 'RETIRAR_HOMOLOGACAO':
                ...
                if (!$condition1) throw new ServerException("ValidatePlanoEntrega", "O plano de entregas não pode ser retirado de homologação porque não se encontra no status AGUARDANDO HOMOLOGAÇÃO. [RN_PENT_AB]");
            ...
            case 'SUSPENDER':
                ...
                $condition4 = $condicoes['gestorLinhaAscendenteUnidadePlano'];
                if (!$condition1) throw new ServerException("ValidatePlanoEntrega", "O plano de entregas não pode ser suspenso porque não se encontra no status ATIVO. [RN_PENT_AD]");

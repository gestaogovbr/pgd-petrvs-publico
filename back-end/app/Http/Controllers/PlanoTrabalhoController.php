...
class PlanoTrabalhoController extends ControllerBase {
    ...
    public function getByUsuario(Request $request) {
        try {
            ...
            Log::error($dataError);
            return response()->json([
                'error' => "Código ".$dataError['code'].
                    ": Ocorreu um erro inesperado ao Pesquisar Planos por Usuário."]);
        }
    }
    ...
    public function metadadosPlano(Request $request) {
        try {
            ...
            $dataError = throwableToArrayLog($e);
            Log::error($dataError);
            return response()->json(['error' => "Codigo ".$dataError['code'].
                ": Ocorreu um erro inesperado ao buscar os Metadados do Plano de Trabalho."]);
        }
    }
    ...
    public function checkPermissions($action, $request, $service, $unidade, $usuario) {
        switch ($action) {
            case 'QUERY':
            case 'GETBYID':
                if (!$usuario->hasPermissionTo('MOD_PTR')){
                    throw new ServerException("CapacidadeSearchText", "O usuário logado não tem permissão para consultar planos de trabalho (MOD_PTR).\n[ver RN_PTR_S]");
                }
                /*
                (RN_PTR_S) CONSULTAR
            ...
            case 'STORE':           //incluir ou alterar um Plano de Trabalho
                ...
                switch ($acao) {
                    case 'INSERT':  // inclusão de um novo Plano de Trabalho
                        if(!$usuario->hasPermissionTo('MOD_PTR_INCL')){
                            throw new ServerException("CapacidadeStore", "Usuário não possui a capacidade de inserir planos de trabalho (MOD_PTR_INCL).\n[ver RN_PTR_V]");
                        }
                        break;
                    case 'EDIT':    // alteração de um Plano de Trabalho
                        if(!$usuario->hasPermissionTo('MOD_PTR_EDT')){
                            throw new ServerException("CapacidadeStore", "Usuário não possui a capacidade de alterar/editar planos de trabalho (MOD_PTR_EDT).\n[ver RN_PTR_M]");
                        }
                        break;
                }
                break;
            ...
            case 'ARQUIVAR':
                ...
                if ($data['arquivar']) {
                    ...
                    */
                    if(!(($condicoes["planoConcluido"] || $condicoes["planoCancelado"]) && !$condicoes["planoArquivado"])){
                        throw new ServerException("ValidatePlanoTrabalho", "Arquivamento não realizado, porque o plano não está no status CONCLUIDO nem CANCELADO, ou já está arquivado.\n[ver RN_PTR_N]");
                    }
                    if(!($condicoes["usuarioEhParticipantePlano"] || $condicoes["gestorUnidadeExecutora"])){
                        throw new ServerException("ValidateUsuario", "Arquivamento não realizado, porque o usuário logado não é participante do plano nem um dos gestores da sua unidade executora.\n[ver RN_PTR_N]");
                    }
                } else {
                    /*
                    ...
                    */
                    ...
                    if(!$condition1) {
                        throw new ServerException("ValidatePlanoTrabalho", "O plano de trabalho não pode ser desarquivado porque não se encontra arquivado.\n[ver RN_PTR_T]");
                    }

                    if(!$condition2) {
                        throw new ServerException("ValidateUsuario", "O plano de trabalho não pode ser desarquivado porque o usuário logado não é o participante do plano nem um dos gestores da sua unidade executora.\n[ver RN_PTR_T]");
                    }
                }
                break;
            case 'ATIVAR':
                ...
                */
                ...
                if(! $condicoes["planoIncluido"]){
                    throw new ServerException("ValidatePlanoTrabalho", "Para ativar um plano de trabalho, ele precisa estar no status INCLUIDO.\n[ver RN_PTR_P]");
                }
                ...
                } elseif ($condicoes["atribuicoesGestorUsuario"]["gestorSubstituto"]) { /* Plano para o gestor substituto da unidade */
                  $validoTabela1 = $condicoes["atribuicoesGestorUsuarioLogado"]["gestor"]
                    || ($condicoes['usuarioEhParticipantePlano']&& $condicoes['usuarioEhParticipanteHabilitado'])
                    || (!$condicoes['usuarioEhParticipantePlano']
                        && $condicoes["atribuicoesGestorUsuarioLogado"]["gestorSubstituto"]);
                } elseif ($condicoes["atribuicoesGestorUsuario"]["gestorDelegado"]) { /* Plano para o gestor delegado da unidade */
                  $validoTabela1 = $condicoes["atribuicoesGestorUsuarioLogado"]["gestor"]
                    || $condicoes["atribuicoesGestorUsuarioLogado"]["gestorSubstituto"]
                    || ($condicoes['usuarioEhParticipantePlano']
                        && $condicoes['usuarioEhParticipanteHabilitado']);
                } elseif ($condicoes['usuarioEhParticipantePlano']) { /* Plano do próprio usuário logado */
                  $validoTabela1 = $condicoes["atribuicoesGestorUsuarioLogado"]["gestor"]
                    || $condicoes["atribuicoesGestorUsuarioLogado"]["gestorSubstituto"] || $condicoes['usuarioEhParticipanteHabilitado'];
                }

                if(!$validoTabela1){
                    throw new ServerException("ValidateUsuario", "Para ativar um plano de trabalho o usuário logado precisa respeitar a ação Ativar da [PTR:TABELA_1].\n[ver RN_PTR_P]");
                }

                if($condicoes["haAssinaturasExigidas"]){
                    throw new ServerException("ValidatePlanoTrabalho", "Para ativar um plano de trabalho, o seu programa não deve exigir nenhuma assinatura no TCR.\n[ver RN_PTR_P]");
                }

                if($condicoes["nrEntregas"] == 0) {
                    throw new ServerException("ValidatePlanoTrabalho", "Para ativar um plano de trabalho, ele precisa possuir ao menos uma entrega.\n[ver RN_PTR_P]");
                }
            ...
            case 'CANCELAR_ASSINATURA':
                ...
                */
                ...
                if(!$condicoes["planoAguardandoAssinatura"]){
                    throw new ServerException("ValidatePlanoTrabalho", "Cancelamento de assinatura não realizado, porque o plano não está no status AGUARDANDO ASSINATURA.\n[ver RN_PTR_Q]");
                }

                if(!$condicoes["usuarioJaAssinouTCR"]){
                    throw new ServerException("ValidateUsuario", "Cancelamento de assinatura não realizado, porque o usuário logado ainda não assinou o TCR.\n[ver RN_PTR_Q]");
                }
            ...
            case 'CANCELAR_PLANO':
                ...
                if(!empty($error)){
                    throw new ServerException("ValidatePlanoTrabalho", $error);
                }
                break;
            ...
            case 'REATIVAR':
                ...
                */
                ...
                if(!$condicoes["planoSuspenso"]){
                    throw new ServerException("ValidatePlanoTrabalho", "O Plano de trabalho não pode ser suspenso porque não se encontra no status SUSPENSO.\n[ver RN_PTR_W]");
                }

                if(!$condicoes["gestorUnidadeExecutora"]){
                    throw new ServerException("ValidateUsuario", "O Plano de trabalho não pode ser suspenso porque O usuário logado não é um dos gestores da Unidade Executora.\n[ver RN_PTR_W]");
                }
            ...
            case 'ENVIAR_ASSINATURA':
                ...
                */
                ...
                if(!$condicoes["planoIncluido"]){
                    throw new ServerException("ValidatePlanoTrabalho", "O Plano de trabalho não pode ser enviado para assinatura porque não se encontra no status INCLUIDO.\n[ver RN_PTR_U]");
                }

                if($condicoes["assinaturaUsuarioExigida"] && $condicoes["usuarioFaltaAssinar"]){
                    throw new ServerException("ValidateUsuario", "O Plano de trabalho não pode ser enviado para assinatura porque o usuário logado ainda não assinou o TCR.\n[ver RN_PTR_U]");
                }

                if(!$condicoes["haAssinaturasFaltantes"]){
                    throw new ServerException("ValidatePlanoTrabalho", "O Plano de trabalho não pode ser enviado para assinatura porque não há assinaturas pendentes.\n[ver RN_PTR_U]");
                }

                if($condicoes["nrEntregas"] == 0){
                    throw new ServerException("ValidatePlanoTrabalho", "O Plano de trabalho não pode ser enviado para assinatura porque ainda não possui nenhuma entrega.\n[ver RN_PTR_U]");
                }
                break;
            ...
            case 'SUSPENDER':
                ...
                */
                ...
                if(!$condicoes["planoAtivo"]){
                    throw new ServerException("ValidatePlanoTrabalho", "O plano de trabalho não pode ser suspenso porque não se encontra no status ATIVO.\n[ver RN_PTR_X]");
                }

                if(!$condicoes["gestorUnidadeExecutora"]){
                    throw new ServerException("ValidateUsuario", "O plano de trabalho não pode ser suspenso porque o usuário logado não é um dos gestores da sua unidade executora.\n[ver RN_PTR_X]");
                }

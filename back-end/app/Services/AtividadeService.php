...
class AtividadeService extends ServiceBase
{
    ...
    */        
    public function validatePeriodo($action, $id, $planoTrabalhoId, $planoTrabalhoEntregaId, $dataDistribuicao, $dataEstipuladaEntrega, $dataInicio) {
        ...
        if(!empty($dataInicio) && (UtilService::asTimestamp($dataInicio) < UtilService::asTimestamp($plano->data_inicio) || UtilService::asTimestamp($dataInicio) > UtilService::asTimestamp($plano->data_fim))) throw new ServerException("ValidateAtividade", "A inicialização da atividade não pode ser anterior ou posterior ao plano de trabalho. (Plano de trabalho: " . UtilService::getDateTimeFormatted($plano->data_inicio) . " - " . UtilService::getDateTimeFormatted($plano->data_fim) . ") [RN_ATV_6]");
        if(UtilService::asTimestamp($plano->data_inicio) > UtilService::asTimestamp($dataDistribuicao) || UtilService::asTimestamp($plano->data_fim) < UtilService::asTimestamp($dataEstipuladaEntrega)) throw new ServerException("ValidateAtividade", "Data da atividade extrapola a do plano de trabalho. (Plano de trabalho: " . UtilService::getDateTimeFormatted($plano->data_inicio) . " - " . UtilService::getDateTimeFormatted($plano->data_fim) . ") [RN_ATV_5]");

...
class UtilService
{
     ...
     * @param   $intervals - esperado um array de intervalos, na seguinte forma:
     *          [['start' => number | DateTime, 'end' => number | DateTime],...['start' => number | DateTime, 'end' => number | DateTime]], 
     *          onde number são Timestamps. A função retorna um array com todas as interseções existentes entre os intervalos recebidos como parâmetro.
     ...
     */
    public static function intersectionsList(array $intervals) : array {
        if($intervals && count($intervals) > 1){
            $isDate = ($intervals[0])['start'] instanceof DateTime;
            $intervalos = array_map(fn($x) => static::asTimeInterval($x), $intervals);
            $result = [];
            for($i = 0; $i < count($intervalos); $i++) {
              for($j = 1; $j < count($intervalos); $j++) {
                if($i != $j && $intervalos[$i]['end'] >= $intervalos[$j]['start'] && $intervalos[$i]['start'] <= $intervalos[$j]['end']) {
                  $intersecao = ['start' => max($intervalos[$i]['start'], $intervalos[$j]['start']), 'end'=> min($intervalos[$i]['end'], $intervalos[$j]['end'])];
                  if(!static::containsInterval($result, $intersecao)) array_push($result, $intersecao);
                }
              }
              array_shift($intervalos);
              $i--;
            }
            if($result && static::intersection($result)) { $isDate ? static::intersectionsList(static::asDateInterval($result)) : static::intersectionsList($result); }
     ...
     *          Retorno esperado da função INTERSECTIONSLIST:    []  
     * 
     * CASO II:
     *    **********
     *        ******************         
     ...
     *                               ['start' => new DateTime('2022-05-01 00:00:00'), 'end' => new DateTime('2022-05-15 00:00:00')]]
     *          Retorno esperado da função INTERSECTIONSLIST:    01/02/22---15/02/22 
     * 
     * CASO III:
     *    **********
     *        ******************         
     ...
     *          Retorno esperado da função INTERSECTIONSLIST:    01/02/22---15/02/22      15/03/22---01/04/22
     * 
     * Intervalos para teste:
     * 01 15  01  15  01  15  01  15  01  15  01  15  01  15  01  15  01  15  01  15  01  15  01  15
     * 01 01  02  02  03  03  04  04  05  05  06  06  07  07  08  08  09  09  10  10  11  11  12  12
     ...
     *          Retorno esperado da função INTERSECTIONSLIST:    01/02/22---15/02/22     15/04/22---01/05/22       
     *
     * CASO V:
     *    **********************************
     *        **************************         
     ...
     *          Retorno esperado da função INTERSECTIONSLIST:    15/02/22---15/04/22    
     * 
     * CASO VI:
     *    **************
     *                        **************         
     ...
     * Retorno esperado da função INTERSECTIONSLIST:   01/02/22---15/02/22   15/02/22---01/03/22   15/02/22---15/02/22   01/04/22---15/04/22   15/03/22---15/04/22 
     *         
     * Intervalos para teste:
     * 01 15  01  15  01  15  01  15  01  15  01  15  01  15  01  15  01  15  01  15  01  15  01  15
     * 01 01  02  02  03  03  04  04  05  05  06  06  07  07  08  08  09  09  10  10  11  11  12  12
     ...
     *          Retorno esperado da função INTERSECTIONSLIST:    Esperado:   01/02/22---01/02/22     15/03/22---01/04/22     15/05/22---15/05/22      
    */

...
class PlanoTrabalhoDesarquivarValidator
{
    ...
    public function validar(string $planoId, string $usuarioLogadoId): PlanoTrabalho
    {
        ...
        if ($plano->data_arquivamento === null) {
            throw new ValidateException('Este Plano de Trabalho não está arquivado.');
        }
        ...
        if (!$this->authorization->isAutorizadoArquivar($plano, $usuario)) {
            throw new ForbiddenException('Usuário não tem permissão para desarquivar este Plano de Trabalho.');
        }

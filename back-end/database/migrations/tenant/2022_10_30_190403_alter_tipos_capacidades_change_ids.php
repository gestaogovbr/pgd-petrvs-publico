...
class AlterTiposCapacidadesChangeIds extends Migration
{
    /* Todas as capacidades até 30/10/2022 */
    public $tiposCapacidades = [ 
        ...
        ["58bf66ce-ba5e-11ec-a5bb-0050569c64a0", "MOD_DMD_NI_CONCL", "Permite concluir demanda não iniciada"],
        ...
        ["59bcba5f-ba5e-11ec-a5bb-0050569c64a0", "MOD_UND_UNIR", "Permite Unificar Unidades"],
    ...
    public function up()
    {
        ...
        DB::delete("DELETE FROM capacidades WHERE tipo_capacidade_id = :antigo", [":antigo" => "a88ba92b-e112-11ec-a5bb-0050569c64a0"]);
        DB::delete("DELETE FROM tipos_capacidades WHERE id = :antigo", [":antigo" => "a88ba92b-e112-11ec-a5bb-0050569c64a0"]);
        foreach($this->tiposCapacidades as $capacidade) {
            DB::update("UPDATE tipos_capacidades SET id = :novo WHERE id = :antigo", [":novo" => $util->uuid($capacidade[1]), ":antigo" => $capacidade[0]]);
        }
        DB::commit();
    }
    ...
    public function down()
    {
        ...
        DB::beginTransaction();
        foreach($this->tiposCapacidades as $capacidade) {
            DB::update("UPDATE tipos_capacidades SET id = :novo WHERE id = :antigo", [":novo" => $capacidade[0], ":antigo" => $util->uuid($capacidade[1])]);
        }
        DB::commit();
    }
}

...
return new class extends Migration
{
    ...
    {
        DB::statement("DROP VIEW `view_relatorio_plano_trabalho_detalhado`");
        ...
        select
            ...
            case
                when `a`.`data_avaliacao` is null then case
                    when `a`.`data_avaliacao` <= cast(`ptc`.`data_conclusao` as date) + interval 20 day then 'Aguardando'
                    else 'Atrasado'
                end
                else case
                    when `a`.`data_avaliacao` <= cast(`ptc`.`data_conclusao` as date) + interval 20 day then 'Registrado no período'
                    else 'Registrado com atraso'
                end

...
test('checkPermissions retorna true', function () {
    $controller = new EnvioPlanoTrabalhoController();
...
test('query lanca excecao quando usuario nao possui permissao', function () {
    ...
    $controller = Mockery::mock(EnvioPlanoTrabalhoController::class)->makePartial();
    $controller->shouldAllowMockingProtectedMethods();
    $controller->shouldReceive('getUsuario')
        ->once()
        ->andReturn($usuario);

    $request = Request::create('/api/EnvioPlanoTrabalho/query', 'POST', [
        'page' => 1,
        'limit' => 10,
        'orderBy' => [],
        'where' => [],

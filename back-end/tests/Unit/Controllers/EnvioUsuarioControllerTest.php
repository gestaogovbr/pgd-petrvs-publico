...
test('checkPermissions retorna true', function () {
    $controller = new EnvioUsuarioController();
...
test('query lanca excecao quando usuario nao possui permissao', function () {
    ...
    $usuario->shouldReceive('hasPermissionTo')
        ->once()
        ->with('MOD_ENVIO_USUARIO')
        ->andReturn(false);

    $controller = Mockery::mock(EnvioUsuarioController::class)->makePartial();
    $controller->shouldAllowMockingProtectedMethods();
    $controller->shouldReceive('getUsuario')
        ->once()
        ->andReturn($usuario);

    $request = Request::create('/api/EnvioUsuario/query', 'POST', [
        'page' => 1,
        'limit' => 10,
        'orderBy' => [],
        'where' => [],

...
let UsuarioIntegranteComponent = class UsuarioIntegranteComponent extends PageFrameBase {
    ...
    constructor(injector) {
        ...
        this.asyncFormValidation = async (form) => {
            ...
            const attrCurador = form.controls.atribuicoes.value.filter((attr) => attr.key == 'CURADOR');
    ...
    async onUnidadeChange(event) {
        ...
        const atribuicoesSelecionadas = this.form?.controls.atribuicoes.value.map((item) => item.key);
        if (unidade_id) {
            const unidade = await this.unidadeDao.getById(unidade_id);
            if (!unidade?.instituidora) {
                atribuicoes = this.lookup.UNIDADE_INTEGRANTE_TIPO.filter(atribuicao => atribuicao.key !== 'CURADOR');
            }
            ...
            if (this.perfil) {
                await this.perfilDao.getById(this.perfil.currentValue).then(perfil => {
                    ...
                    if (perfil.nivel === 6) {
                        ...
                        atribuicoes = atribuicoes.filter(atribuicao => atribuicao.key !== 'CURADOR' && atribuicao.key !== 'GESTOR_SUBSTITUTO');
                        perfilColaborador = true;
                    }
                });
            }
            const isLotado = atribuicoesSelecionadas.includes('LOTADO');
            atribuicoes = atribuicoes.filter(atribuicao => atribuicao.key == 'COLABORADOR' ? !isLotado : !atribuicoesSelecionadas.includes(atribuicao.key));
            if (!unidade?.executora && !perfilColaborador) {
                atribuicoes = this.lookup.UNIDADE_INTEGRANTE_TIPO.filter(atribuicao => atribuicao.key !== 'COLABORADOR');
            }
    ...
    deleteItemHandle(row) {
        ...
        const isLotado = this.form?.controls.atribuicoes.value.filter((val) => val.key == "LOTADO").length > 0;
        const atribuicoes = this.lookup.UNIDADE_INTEGRANTE_TIPO;
        this.atribuicoes = atribuicoes.filter(atribuicao => atribuicao.key == 'COLABORADOR' ? !isLotado : atribuicao.key == atribuicaoExcluida || this.atribuicoes.includes(atribuicao));
        return true;
    }
    ;
    /**
...
], UsuarioIntegranteComponent.prototype, "grid", void 0);
__decorate([
    ViewChild('unidade', { static: false })
], UsuarioIntegranteComponent.prototype, "unidade", void 0);
__decorate([
    ViewChild('perfil', { static: false })
], UsuarioIntegranteComponent.prototype, "perfil", void 0);
__decorate([
    Input()
], UsuarioIntegranteComponent.prototype, "control", null);
__decorate([
    Input()
], UsuarioIntegranteComponent.prototype, "entity", null);
__decorate([
    Input()
], UsuarioIntegranteComponent.prototype, "noPersist", null);
__decorate([
    Input()
], UsuarioIntegranteComponent.prototype, "parent", void 0);
__decorate([
    Input()
], UsuarioIntegranteComponent.prototype, "readOnly", void 0);
UsuarioIntegranteComponent = __decorate([
    Component({
...
export { UsuarioIntegranteComponent };
//# sourceMappingURL=usuario-integrante.component.js.map
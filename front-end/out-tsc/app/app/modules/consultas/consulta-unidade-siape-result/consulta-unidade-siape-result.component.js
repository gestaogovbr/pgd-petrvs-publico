...
let ConsultaUnidadeSiapeResultComponent = class ConsultaUnidadeSiapeResultComponent extends PageFormBase {
        ...
        this.toolbarButtons = [
            {
                ...
                onClick: async () => {
                    this.loading = true;
                    try {
                        const response = await firstValueFrom(this.dao.exportarUnidadeSIAPE(this.codigoUnidade));
            ...
            },
            {
                ...
                onClick: async () => {
                    ...
                    if (confirm) {
                        ...
                        try {
                            this.dao.sincronizarSIAPE(this.codigoUnidade)
    ...
    async downloadSiape() {
        this.loading = true;
        try {
            const response = await firstValueFrom(this.dao.baixaLogSiape(this.codigoUnidade));
            const contentType = response.type;
            const dataCriacao = new Date().toISOString().slice(0, 10);
            const extensoes = {
                'application/xml': 'xml',
    ...
    async obterRelatorioProcessamentoSafely() {
        try {
            const response = await firstValueFrom(this.dao.relatorioProcessamentoSIAPE(this.codigoUnidade));
            return {
    ...
    async mostrarResumo(resumo, titulo, relatorio, relatorioCargaId) {
        ...
        if (this.resumoTpl) {
            ...
            const dialogResult = await this.dialog.template({ title: titulo, modalWidth: 700 }, this.resumoTpl, buttons, { resumo: resumo, relatorio: relatorio }).asPromise();
...
], ConsultaUnidadeSiapeResultComponent.prototype, "editableForm", void 0);
__decorate([
    ViewChild('resumoTpl')
], ConsultaUnidadeSiapeResultComponent.prototype, "resumoTpl", void 0);
ConsultaUnidadeSiapeResultComponent = __decorate([
    Component({
...
export { ConsultaUnidadeSiapeResultComponent };
//# sourceMappingURL=consulta-unidade-siape-result.component.js.map
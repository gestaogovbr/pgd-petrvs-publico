...
let AtividadeService = class AtividadeService {
    constructor(lookup, lex, calendar, comentario, auth, util, go, dialog, dao, modalidadePgd) {
        this.lookup = lookup;
        this.lex = lex;
        ...
        this.dialog = dialog;
        this.dao = dao;
        this.modalidadePgd = modalidadePgd;
        ...
        this.dynamicOptions = (row, metadata) => {
            ...
            const BOTAO_COMENTARIOS = { label: "Comentários", icon: "bi bi-chat-left-quote", onClick: (atividade) => this.go.navigate({ route: ['uteis', 'comentarios', 'ATIVIDADE', atividade.id, 'new'] }, this.modalRefreshId(metadata, atividade)) };
            const BOTAO_CLONAR = { label: "Clonar", icon: "bi bi-stickies", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'clonar'] }, this.modalRefresh(metadata)) };
            const BOTAO_ALTERAR = { label: "Alterar atividade", icon: "bi bi-pencil-square", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'edit'] }, this.modalRefreshId(metadata, atividade)) };
            const BOTAO_EXCLUIR = { label: "Excluir atividade", icon: "bi bi-trash", onClick: this.delete.bind(this, metadata) };
            const BOTAO_PRORROGAR_PRAZO = { label: "Prorrogar prazo", icon: "bi bi-skip-end-circle", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'prorrogar'] }, this.modalRefreshId(metadata, atividade)) };
            const BOTAO_INICIAR = { label: "Iniciar", id: "INICIADO", icon: "bi bi-play-circle", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'iniciar'] }, this.modalRefreshId(metadata, atividade)) };
            ...
            const BOTAO_ALTERAR_INICIO = { label: "Alterar inicio", icon: "bi bi-play-circle", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'iniciar'] }, this.modalRefreshId(metadata, atividade)) };
            const BOTAO_PAUSAR = { label: "Pausar", id: "PAUSADO", icon: "bi bi-pause-circle", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'pausar'] }, this.modalRefreshId(metadata, atividade)) };
            const BOTAO_REINICIAR = { label: "Reiniciar", id: "INICIADO", icon: "bi bi-play-circle", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'pausar'], params: { reiniciar: true } }, this.modalRefreshId(metadata, atividade)) };
            const BOTAO_CONCLUIR = { label: "Concluir", id: "CONCLUIDO", icon: "bi bi-check-circle", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'concluir'] }, this.modalRefreshId(metadata, atividade)) };
            const BOTAO_ALTERAR_CONCLUSAO = { label: "Alterar conclusão", icon: "bi bi-check-circle", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'concluir'] }, this.modalRefreshId(metadata, atividade)) };
        ...
        this.dynamicButtons = (row, metadata) => {
            ...
            const isResponsavel = this.auth.usuario?.id == atividade.usuario_id;
            const lastConsolidacao = this.lastConsolidacao(row.metadados?.consolidacoes);
            const BOTAO_ALTERAR_AVALIACAO = { hint: "Alterar avaliação", icon: "bi bi-check-all", color: "btn-outline-danger", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'avaliar'] }, this.modalRefreshId(metadata, atividade)) };
            const BOTAO_INFORMACOES = { label: "Informações", icon: "bi bi-info-circle", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'consult'] }, { modal: true }) };
            const BOTAO_INICIAR = { hint: "Iniciar", icon: "bi bi-play-circle", color: "btn-outline-secondary", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'iniciar'] }, this.modalRefreshId(metadata, atividade)) };
            const BOTAO_REINICIAR = { hint: "Reiniciar", icon: "bi bi-play-circle", color: "btn-outline-secondary", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'pausar'], params: { reiniciar: true } }, this.modalRefreshId(metadata, atividade)) };
            const BOTAO_CONCLUIR = { hint: "Concluir", icon: "bi bi-check", color: "btn-outline-success", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'concluir'] }, this.modalRefreshId(metadata, atividade)) };
            ...
            const BOTAO_ALTERAR_CONCLUSAO = { hint: "Alterar conclusão", icon: "bi bi-check-circle", onClick: (atividade) => this.go.navigate({ route: ['gestao', 'atividade', atividade.id, 'concluir'] }, this.modalRefreshId(metadata, atividade)) };
            if (!metadata?.disabled) {
    ...
    getStatus(row, consolidacao) {
        ...
        if (atividade.metadados?.atrasado)
    ...
    temposAtividade(row, extra, despendidoClick) {
        ...
        if (row.metadados && row.metadados.extra?.lastUpdate != this.auth.unidadeHora) {
            ...
            if (this.modalidadePgd.atividadeEsforco(planoTrabalho?.modalidade_pgd))
    ...
    buildChecklist(tipoAtividade, checklistControl) {
        ...
        if (checklistControl)
            checklistControl.setValue(checks);
        return checks;
    }
    comentarioAtividade(tipoAtividade, comentariosControl) {
        ...
        if (tipoAtividade?.comentario?.length) {
            ...
            if (comentariosControl)
                comentariosControl.setValue(comentarios);
        }
    }
    modalRefreshId(metadata, entity) {
        return { modal: true, modalClose: async (modalResult) => metadata.refreshId(entity.id) };
    }

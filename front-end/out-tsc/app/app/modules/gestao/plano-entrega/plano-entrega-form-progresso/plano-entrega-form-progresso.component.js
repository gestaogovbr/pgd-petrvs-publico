...
let PlanoEntregaFormProgressoComponent = class PlanoEntregaFormProgressoComponent extends PageFormBase {
    ...
    async initializeData(form) {
        ...
        this.planoEntregaEntrega = this.planoEntregaEntregaId ? await this.planoEntregaEntregaDao.getById(this.planoEntregaEntregaId, ['entrega']) : undefined;
        ...
        this.checklist = Array.isArray(this.planoEntregaEntrega?.checklist)
            ? JSON.parse(JSON.stringify(this.planoEntregaEntrega.checklist))
            : [];
        await this.loadData(this.entity, form);
    }
    async saveData(form) {
        let progresso = this.util.fill(new PlanoEntregaEntregaProgresso(), this.entity);
        ...
        if (this.planoEntregaEntrega?.id) {
            try {
                await this.planoEntregaEntregaDao.update(this.planoEntregaEntrega.id, { checklist: this.checklist });
            }
            catch (e) {
...
], PlanoEntregaFormProgressoComponent.prototype, "editableForm", void 0);
PlanoEntregaFormProgressoComponent = __decorate([
    Component({
...
export { PlanoEntregaFormProgressoComponent };
//# sourceMappingURL=plano-entrega-form-progresso.component.js.map
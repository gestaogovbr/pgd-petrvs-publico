...
let PlanoEntregaListComponent = class PlanoEntregaListComponent extends PageListBase {
    constructor(injector) {
        ...
        this.execucao = false;
        this.planejamento = false;
        this.habilitarAdesaoToolbar = false;
        this.toolbarButtons = [];
        this.botoes = [];
        this.routeStatus = { route: ["uteis", "status"] };
        this.DATAS_FILTRO = [
        ...
        */
            if (this.filter?.controls.principais.value) {
                ...
                if (this.auth.isGestorAlgumaAreaTrabalho()) {
                    ...
                    let w2 = unidadesUsuarioEhGestor
            ...
            if (this.isModal) {
                ...
                if (this.planejamento)
                    status.push('INCLUIDO', 'HOMOLOGANDO', 'ATIVO');
                if (this.execucao)
                    status.push('ATIVO', 'CONCLUIDO');
                if (this.avaliacao)
                    status.push('CONCLUIDO', 'AVALIADO');
        ...
        this.BOTAO_ALTERAR = {
            ...
            onClick: (planoEntrega) => this.go.navigate({ route: ["gestao", "plano-entrega", planoEntrega.id, "edit"] }, this.modalRefreshId(planoEntrega)),
        };
        this.BOTAO_CLONAR = {
            label: "Clonar",
            icon: "bi bi-copy",
            color: "btn-outline-primary",
            onClick: (planoEntrega) => {
                this.dialog
                    .alert("Atenção!", "Não serão clonadas as metas e as entregas cujos vínculos com objetivos estratégicos, processos de cadeia de valor ou entregas de unidade superior não sejam mais possíveis.")
                    .then(() => {
                    this.go.navigate({ route: ["gestao", "plano-entrega", planoEntrega.id, "clone"] }, this.modalRefreshId(planoEntrega));
                });
            },
        };
        this.BOTAO_ARQUIVAR = {
        ...
        this.BOTAO_CONSULTAR = {
            ...
            onClick: (planoEntrega) => this.go.navigate({ route: ["gestao", "plano-entrega", planoEntrega.id, "consult"] }, { modal: true }),
        };
    ...
    onGridLoad(rows) {
        ...
        rows?.forEach((v) => {
            ...
            if (planoEntrega.avaliacao) {
                planoEntrega.avaliacao.tipo_avaliacao = extra?.tipos_avaliacoes?.find((x) => x.id == planoEntrega.avaliacao.tipo_avaliacao_id);
            }
        });
    }
    ...
    */
    ...
    dynamicButtons(row) {
        ...
        const dataInativacao = row.unidade?.data_inativacao;
    ...
    botaoAtendeCondicoes(botao, planoEntrega) {
        ...
        */
                ...
                let condicao1 = ["INCLUIDO", "HOMOLOGANDO"].includes(this.planoEntregaService.situacaoPlano(planoEntrega)) &&
                    (this.unidadeService.isGestorUnidade(planoEntrega.unidade) ||
                        this.auth.isLotacaoUsuario(planoEntrega.unidade));
                let condicao2 = (planoEntrega.unidade?.instituidora == 1
                    ? this.unidadeService.isGestorUnidade(planoEntrega.unidade?.id)
                    : this.unidadeService.isGestorUnidade(planoEntrega.unidade?.unidade_pai_id)) && this.auth.hasPermissionTo("MOD_PENT_EDT_FLH");
                let condicao3 = this.auth.isIntegrante("HOMOLOGADOR_PLANO_ENTREGA", planoEntrega.unidade?.unidade_pai_id);
                let condicao4 = this.planoEntregaService.situacaoPlano(planoEntrega) == "ATIVO" &&
                    this.auth.isLotacaoUsuario(planoEntrega.unidade) &&
        ...
        */
                return (["CONCLUIDO", "AVALIADO", "CANCELADO"].includes(this.planoEntregaService.situacaoPlano(planoEntrega)) &&
                    (this.unidadeService.isGestorUnidade(planoEntrega.unidade) ||
                        (this.auth.isLotacaoUsuario(planoEntrega.unidade) &&
        ...
        */
                if (!this.avaliacao)
                    return false;
                let condic1 = planoEntrega.unidade?.instituidora == 1
                    ? this.unidadeService.isGestorUnidade(planoEntrega.unidade?.id)
                    : this.unidadeService.isGestorUnidade(planoEntrega.unidade?.unidade_pai_id);
                let condic3 = this.auth.isLotacaoUsuario(planoEntrega.unidade?.unidade_pai) &&
                    this.auth.hasPermissionTo("MOD_PENT_AVAL");
                let condic4 = this.auth.isGestorLinhaAscendente(planoEntrega.unidade) &&
                    this.auth.hasPermissionTo("MOD_PENT_AVAL_SUBORD");
                return (this.planoEntregaService.situacaoPlano(planoEntrega) == "CONCLUIDO" &&
        ...
        */
                return (this.planoEntregaService.situacaoPlano(planoEntrega) == "AVALIADO" &&
                    (planoEntrega.unidade?.instituidora == 1
                        ? this.unidadeService.isGestorUnidade(planoEntrega.unidade?.id)
                        : this.unidadeService.isGestorUnidade(planoEntrega.unidade?.unidade_pai_id) ||
                            this.auth.isIntegrante("AVALIADOR_PLANO_ENTREGA", planoEntrega.unidade?.id)));
        ...
        */
                return (!this.execucao &&
                    ...
                    (planoEntrega.unidade?.instituidora == 1
                        ...
                        : this.unidadeService.isGestorUnidade(planoEntrega.unidade?.unidade_pai_id) ||
                            ...
                            this.auth.isIntegrante("HOMOLOGADOR_PLANO_ENTREGA", planoEntrega.unidade.unidade_pai_id)));
          ...
          */
                return (this.auth.hasPermissionTo("MOD_PENT_CNC") &&
                    ["INCLUIDO", "HOMOLOGANDO", "ATIVO"].includes(this.planoEntregaService.situacaoPlano(planoEntrega)) &&
                    (this.unidadeService.isGestorUnidade(planoEntrega.unidade?.id) ||
                        this.unidadeService.isGestorUnidade(planoEntrega.unidade?.unidade_pai_id)));
        ...
        */
                return (this.planoEntregaService.situacaoPlano(planoEntrega) == "ATIVO" &&
                    this.execucao &&
                    (this.unidadeService.isGestorUnidade(planoEntrega.unidade) ||
                        (this.auth.isLotacaoUsuario(planoEntrega.unidade) &&
        ...
        */
          ...
          - o plano precisa estar arquivado, e:
              ...
              - a Unidade do plano (Unidade B) precisa ser a Unidade de lotação do usuário logado e ele possuir a capacidade "MOD_PENT_ARQ";
        */
                return (this.planoEntregaService.situacaoPlano(planoEntrega) == "ARQUIVADO" &&
                    (this.unidadeService.isGestorUnidade(planoEntrega.unidade) ||
                        (this.auth.isLotacaoUsuario(planoEntrega.unidade) &&
                            this.auth.hasPermissionTo("MOD_PENT_ARQ"))));
        ...
        */
                return (!this.execucao &&
                    ...
                    ["INCLUIDO", "HOMOLOGANDO"].includes(this.planoEntregaService.situacaoPlano(planoEntrega)) &&
                    (this.unidadeService.isGestorUnidade(planoEntrega.unidade) ||
                        this.auth.isLotacaoUsuario(planoEntrega.unidade)));
        ...
        */
                ...
                let condition2 = planoEntrega.unidade?.instituidora == 1
                    ? this.unidadeService.isGestorUnidade(planoEntrega.unidade?.id)
                    : this.unidadeService.isGestorUnidade(planoEntrega.unidade?.unidade_pai_id);
                let condition3 = this.auth.isLotacaoUsuario(planoEntrega.unidade?.unidade_pai) &&
        ...
        */
                return (this.unidadeService.isGestorUnidade(planoEntrega.unidade) &&
                    this.auth.hasPermissionTo("MOD_PENT_AVAL_SUBORD"));
        ...
        */
                return (this.planoEntregaService.situacaoPlano(planoEntrega) == "SUSPENSO" &&
                    (this.unidadeService.isGestorUnidade(planoEntrega.unidade) ||
                        (this.auth.isLotacaoUsuario(planoEntrega.unidade) &&
        ...
        */
            ...
            case this.BOTAO_CLONAR:
                const unidadeAtiva = !planoEntrega.unidade?.data_inativacao ||
                    planoEntrega.unidade?.data_inativacao === "";
                return (this.auth.hasPermissionTo("MOD_PENT_INCL") &&
                    !["HOMOLOGANDO"].includes(this.planoEntregaService.situacaoPlano(planoEntrega)) &&
                    unidadeAtiva);
        }
        return false;
    }
    ...
    async cancelarAvaliacao(planoEntrega) {
        /*this.go.navigate(this.routeStatus, {
...
], PlanoEntregaListComponent.prototype, "grid", void 0);
PlanoEntregaListComponent = __decorate([
    Component({
...
export { PlanoEntregaListComponent };
//# sourceMappingURL=plano-entrega-list.component.js.map
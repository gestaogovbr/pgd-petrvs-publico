...
let PlanoTrabalhoListComponent = class PlanoTrabalhoListComponent extends PageListBase {
    constructor(injector) {
        ...
        this.orderByHierarquia = [['numero', 'desc']];
        this.groupByPadrao = [
            { field: "unidade.sigla", label: "Unidade" }
        ];
        this.orderByPadrao = [["numero", "desc"]];
        ...
        this.filterWhere = (filter) => {
            ...
            if (this.grid)
                this.grid.groupBy = this.groupBy;
            return result;
        };
        this.dynamicMultiselectMenu = (multiselected) => {
            ...
            Object.entries(multiselected).forEach(([key, value]) => {
                if (!this.planoTrabalhoService.needSign(value))
                    assinar = false;
            });
            if (assinar)
                menu.push({
        ...
        this.planoTrabalhoService =
            injector.get(PlanoTrabalhoService);
        ...
        this.BOTAO_TERMOS = {
            ...
            onClick: ((row) => this.go.navigate({ route: ["uteis", "documentos", "TCR", row.id] }, {
        ...
        this.BOTAO_CLONAR = { label: "Clonar", icon: "bi bi-copy", color: "btn-outline-primary", onClick: (planoTrabalho) => {
    ...
    dynamicButtons(row) {
        ...
        */
        ...
        if (!result.length)
            result.push(this.BOTAO_INFORMACOES);
        return result;
    }
    filterClear(filter) {
    ...
    botaoAtendeCondicoes(botao, planoTrabalho) {
        ...
        let haAssinaturasFaltantes = !!assinaturasFaltantes.participante?.length ||
        ...
        let planoAguardandoAssinatura = this.planoTrabalhoService.situacaoPlano(planoTrabalho) ==
        ...
        let planoAvaliado = this.planoTrabalhoService.situacaoPlano(planoTrabalho) == "AVALIADO";
        ...
        if (botao == this.BOTAO_INFORMACOES &&
      ...
      */
            ...
            if (planoDeletado) {
                ...
                let gestorUnidadeSuperior = planoTrabalho._metadata?.atribuicoesLogadoUnidadeSuperior.gestor ||
                    planoTrabalho._metadata?.atribuicoesLogadoUnidadeSuperior
                        .gestorSubstituto;
                let gestorLogado = planoTrabalho._metadata?.atribuicoesLogado.gestor ||
            ...
            */
                        return ((planoConcluido || planoCancelado) &&
                            !planoArquivado &&
            ...
            */
                    ...
                    case this.BOTAO_CLONAR:
                        const inativacao = planoTrabalho.unidade?.data_inativacao;
    ...
    contadorAssinaturas(planoTrabalho) {
    ...
    assinar(planoTrabalho) {
        ...
        if (!documentos.length) {
            ...
            this.documentoService
                .sign(documentos)
                .then(() => (this.grid?.query || this.query).refreshId(planoTrabalho.id));
        }
    }
...
], PlanoTrabalhoListComponent.prototype, "grid", void 0);
PlanoTrabalhoListComponent = __decorate([
    Component({
...
export { PlanoTrabalhoListComponent };
//# sourceMappingURL=plano-trabalho-list.component.js.map
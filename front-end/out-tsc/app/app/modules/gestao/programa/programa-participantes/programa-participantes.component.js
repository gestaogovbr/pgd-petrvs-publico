...
let ProgramaParticipantesComponent = class ProgramaParticipantesComponent extends PageListBase {
    constructor(injector) {
        super(injector, Usuario, UsuarioDaoService);
        this.injector = injector;
        this.multiselectMenu = [];
        this.programa = null;
        this.BOTAO_HABILITAR = { label: this.lex.translate("Habilitar"), hint: this.lex.translate("Habilitar"), icon: "bi bi-person-check-fill", color: "btn-outline-success", onClick: this.habilitarParticipante.bind(this) };
        this.BOTAO_DESABILITAR = { label: this.lex.translate("Desabilitar"), hint: this.lex.translate("Desabilitar"), icon: "bi bi-person-x-fill", color: "btn-outline-danger", onClick: this.desabilitarParticipante.bind(this) };
        this.condicoes = [
            {
                key: '1',
        ...
        this.filterWhere = (filter) => {
            ...
            if (form.unidade_id?.length)
                result.push(["lotacao", "==", form.unidade_id]);
            if (form.nome_usuario?.length)
                result.push(["nome", "like", "%" + form.nome_usuario.trim().replace(" ", "%") + "%"]);
        ...
        this.programaParticipanteDao = injector.get(ProgramaParticipanteDaoService);
        ...
        if (this.auth.hasPermissionTo('MOD_PART_HAB'))
            this.multiselectMenu.push({
                icon: "bi bi-person-check-fill",
                label: this.lex.translate("Habilitar"),
                color: "btn-outline-success",
                onClick: this.habilitarParticipantes.bind(this)
            });
        if (this.auth.hasPermissionTo('MOD_PART_DESAB'))
            this.multiselectMenu.push({
    ...
    dynamicButtons(row) {
        ...
        if (row.usuario_externo)
            return result;
        if (this.auth.hasPermissionTo('MOD_PART_HAB') && !this.isHabilitado(row))
            result.push(this.BOTAO_HABILITAR);
        if (this.auth.hasPermissionTo('MOD_PART_DESAB') && this.isHabilitado(row))
            result.push(this.BOTAO_DESABILITAR);
    ...
    async habilitarParticipante(row) {
    ...
    async desabilitarParticipante(row) {
        let desabilitar = await this.dialog.confirm("Desabilitar ?", "Deseja DESABILITAR " + this.lex.translate("o servidor") + " " + row.nome.toUpperCase() + " " + this.lex.translate("do programa") + " " + (this.programa?.nome).toUpperCase() + " ?");
        if (desabilitar) {
            let suspender = false;
    ...
    async desabilitarParticipantes() {
        let idsProgramasParticipantes = Object.keys(this.grid.multiselected);
        this.dialog.confirm("Desabilitar ?", "Deseja DESABILITAR, " + this.lex.translate("do programa") + " " + (this.programa?.nome).toUpperCase() + " todos " + this.lex.translate("os usuários") + " selecionados ?").then(async (desabilitar) => {
            if (desabilitar) {
                const self = this;
...
], ProgramaParticipantesComponent.prototype, "grid", void 0);
__decorate([
    ViewChild("programaSearch", { static: false })
], ProgramaParticipantesComponent.prototype, "programaSearch", void 0);
ProgramaParticipantesComponent = __decorate([
    Component({
...
export { ProgramaParticipantesComponent };
//# sourceMappingURL=programa-participantes.component.js.map
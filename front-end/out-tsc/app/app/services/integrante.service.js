...
let IntegranteService = class IntegranteService {
    constructor(injector) {
        ...
        this.siapeBlacklistDao = this.injector.get(SiapeBlacklistServidorDaoService);
    }
    /**
     *
     * @param atribuicoes
    ...
    haAlteracaoGestor(novasAtribuicoes, row, itensGrid, nomeOuSigla) {
        ...
        if (!ehLotadoNaNovaGerencia && indiceItemLotacao >= 0)
            msg += "Por consequência, sua lotação será alterada de " + itensGrid[indiceItemLotacao].unidade_sigla?.toUpperCase() + " para " + row.unidade_sigla?.toUpperCase() + ".";
   ...
   */
    haAlteracaoGerencia(novasAtribuicoes, row, itensGrid, nomeOuSigla) {
        ...
        let indiceItemLotacao = itensGrid.findIndex((item, index, obj) => item.atribuicoes.includes('LOTADO'));
        if (!ehLotadoNestaUnidade && indiceItemLotacao >= 0)
            msg += "Como é lotado em outra unidade, a lotação de " + row.usuario_nome?.toUpperCase() + " será alterada para " + this.lex.translate("a Unidade") + " - " + nomeOuSigla.toUpperCase() + ".";
    ...
    haAlteracaoLotacao(form, row, itensGrid, nomeOuSigla) {
        ...
        let indiceItemLotacao = itensGrid.findIndex((item, index, obj) => item.atribuicoes.includes('LOTADO'));
        if (!novasAtribuicoes.includes('LOTADO') || indiceItemLotacao == -1)
            return [false, -1, "", ""];
    ...
    ehPermitidoApagar(atribuicao) {
        ...
        (async () => {
            if (!permitidoApagar)
                await this.dialog.alert("NÃO PERMITIDO!", msg);
        })();
    ...
    async consultarBlacklistCpf(cpf) {
        try {
            const response = await this.siapeBlacklistDao.queryByCpf(cpf).toPromise();
            const rows = (response?.rows || []).map((r) => new SiapeBlacklistServidor(r));
...
IntegranteService = __decorate([
    Injectable({
        providedIn: 'root',
    })
], IntegranteService);
export { IntegranteService };
//# sourceMappingURL=integrante.service.js.map
...
const routes: Routes = [
  ...
  { path: 'impersonate', component: TesteImpersonateComponent, canActivate: [AuthGuard], resolve: { config: ConfigResolver }, runGuardsAndResolvers: 'always', data: { title: "Personificar", permission: 'MENU_DEV_ACESSO', requireAdmin: true } },
  ...
  { path: 'relatos/lotacao', loadChildren: () => import('./modules/relatos/lotacao/relato-lotacao.module').then(m => m.RelatoLotacaoModule), canActivate: [AuthGuard] },
  {
    ...
    data: { breadcrumb: 'Consulta de Envio' }
  },

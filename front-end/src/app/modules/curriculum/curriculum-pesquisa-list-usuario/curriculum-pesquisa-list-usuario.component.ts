...
export class CurriculumPesquisaListUsuarioComponent extends PageFrameBase {
  ...
  public set items(value: CurriculumProfissional[]) {
    if(this.items != value) {
      this.gridControl.value.curriculum = value;
      if(this.viewInit) this.cdRef.detectChanges();
    }    
  }
  public get items(): CurriculumProfissional[] {
    if (!this.gridControl.value.curriculum) this.gridControl.setValue(new CurriculumProfissional());
    if (!this.gridControl.value.curriculum) this.gridControl.value.curriculum = [];
    return this.gridControl.value.curriculum;
  }

...
import { EnvioPlanoEntregaListFilters, EnvioPlanoEntregaRow } from '../domain/types';
...
@Component({
  selector: 'app-envio-plano-entrega-list-page',
  standalone: true,
...
export class EnvioPlanoEntregaListPage implements OnInit {
  readonly facade = inject(EnvioPlanoEntregaListFacade);
  ...
  private readonly FILTER_KEY = 'envio-plano-entrega:filters';
  ...
  private restoreFilters(): void {
    const parsed = this.filterStorage.load<Partial<EnvioPlanoEntregaListFilters & { unidade_id?: unknown }>>(
      this.FILTER_KEY,
    );
    if (!parsed) return;
    const status =
    ...
    this.filters.patchValue(
      {
        ...
        nome: parsed.nome !== undefined && parsed.nome !== null ? String(parsed.nome) : '',
  ...
  private buildFilters(): EnvioPlanoEntregaListFilters {
    const raw = this.filters.getRawValue();
    const out: EnvioPlanoEntregaListFilters = {};
  ...
  hasTentativaBeforeAgendamento(row: EnvioPlanoEntregaRow): boolean {
    const a = row.data_agendamento_envio;
    const t = row.data_tentativa_envio;
    return !!a && !!t && a > t;
  }

  hasConclusaoBeforeAgendamento(row: EnvioPlanoEntregaRow): boolean {
    const a = row.data_agendamento_envio;
    const c = row.data_conclusao_envio;
    return !!a && !!c && a > c;
  }

  hasEnvioBeforeAgendamento(row: EnvioPlanoEntregaRow): boolean {
    const a = row.data_agendamento_envio;

  ...
  private montarHtml(cabecalho: PdfCabecalho, indicadores: PdfIndicador[]): string {
    const indicadoresHtml = indicadores.map(ind => {
      const grafico = ind.canvasEl
        ? `<div style="display: flex; align-items: stretch; gap: 8px;">
            <img src="${ind.canvasEl.toDataURL('image/png')}" style="flex: 1; height: auto;" />
      ...
      return `
        <div style="margin-bottom: 24px; border: 1px solid #ddd; padding: 16px; page-break-inside: avoid;">
          ...
          ${grafico}
          <div style="margin-top: 8px;">${legendaHtml}</div>
    ...
    return `
      ...
      <head>
        ...
        <style>
          body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; margin: 20px; color: #333; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
      ...
      <body>
        ...
        ${cabecalho.periodo ? `<p style="margin: 2px 0; font-size: 12px;"><strong>Período:</strong> ${cabecalho.periodo}</p>` : ''}
        <p style="margin: 2px 0 16px 0; font-size: 12px;"><strong>Data/hora de geração:</strong> ${new Date().toLocaleString('pt-BR')}</p>

...
export class PlanoEntregaFormComponent extends PageFormBase<PlanoEntrega, PlanoEntregaDaoService> {
  ...
  public formValidation = (form?: FormGroup) => {
    ...
    if(!programa) {
      ...
      if (programa.prazo_execucao > 0 && diffDays > programa.prazo_execucao) return "A data de início e término" + this.lex.translate("Plano de Entrega", false, true) + " deve respeitar o período previsto no " + this.lex.translate("Programa");
      for(let entrega of entregas) {
        if(entrega.inicio < inicio) return "A " + this.lex.translate("entrega") + " '" + entrega.descricao + "' possui data inicial anterior a" + this.lex.translate("plano de entrega", false, true) + ": " + this.util.getDateFormatted(inicio);
        if(entrega.fim > fim) return "A " + this.lex.translate("entrega") + " '" + entrega.descricao + "' possui data fim posterior a" + this.lex.translate("plano de entrega", false, true) + ": " + this.util.getDateFormatted(fim);
      }
    }
    return undefined;
  }

...
@Component({
  selector: 'plano-trabalho-termo-adesao',
  templateUrl: './plano-trabalho-termo-adesao.component.html',
  styleUrls: ['./plano-trabalho-termo-adesao.component.scss']
})
export class PlanoTrabalhoTermoAdesaoComponent extends PageBase implements OnInit {
  ...
  @Input() set plano(value: PlanoTrabalho | undefined) {
    if(this._plano != value) {
      this._plano = value;
/*       if(this._plano?.atividades?.length) {
        this.atividades = this._plano?.atividades.map(x => x.tipo_atividade!) || [];
      } else {
        this.loadAtividades();
      } */
      this.cdRef.detectChanges();
  ...
  get plano(): PlanoTrabalho | undefined {
    return this._plano;
  }
  ...
  public getTempoTeletrabalho(atividade: Atividade): string {
    ...
    const fator_ganho_produtivade = 1;

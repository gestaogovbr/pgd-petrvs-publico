...
export class ForcaDeTrabalhoReportAreaComponent extends PageReportBase<Usuario, UsuarioDaoService> {
  ...
  public opcoesGraficoBarrasEmpilhado: ChartOptions = {
    scales: {
      xAxes: [{
        stacked: true,
        ticks: {
...
/*       datalabels: {
  ...
  public opcoesGraficoBarrasNaoEmpilhado: ChartOptions = {
    scales: {
      xAxes: [{
        //stacked: true,
        ticks: {
...
/*       datalabels: {
  ...
  public opcoesGraficoPizza: ChartOptions = {
    responsive: true,
  };
/*   public labelsGraficoPizza: Label[] = [['Download', 'Sales'], ['In', 'Store', 'Sales'], 'Mail Sales'];
  ...
  public async report(filter: any) {
    ...
    let result = await this.unidadeDao.metadadosArea(this.parametros.unidade_id);
    ...
    result['graficoAreaGeral'] = this.obterDadosGrafico(result['dadosArea'], 'GERAL');
    result['graficoAreaDetalhado'] = this.obterDadosGrafico(result['dadosArea'], 'DETALHADO');

    result['graficoUnidadeGeral'] = this.obterDadosGrafico(result['dadosUnidade'], 'GERAL');
    result['graficoUnidadeDetalhado'] = this.obterDadosGrafico(result['dadosUnidade'], 'DETALHADO');
    return [result];
  }

  public obterDadosGrafico(metadados: MetadadosUnidade, tipo: 'GERAL' | 'DETALHADO') {
      let $dados: any = null;

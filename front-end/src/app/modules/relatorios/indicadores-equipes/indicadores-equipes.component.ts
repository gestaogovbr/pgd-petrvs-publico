...
import { IndicadorEquipeDaoService, IndicadoresHorasFilter } from "src/app/dao/indicador-equipe-dao.service";
...
export class IndicadorEquipeComponent extends RelatorioBaseComponent<IndicadorEquipe, IndicadorEquipeDaoService> {
  ...
  private horasSubscription?: Subscription;
  ...
  public async ngOnInit() {
    ...
    this.horasSubscription = this.dao!.queryHoras(this.buildHorasFilter()).subscribe({
      next: (result) => {
  ...
  private buildHorasFilter(): IndicadoresHorasFilter {
    const form = this.filter?.value;
    const filter: IndicadoresHorasFilter = {
      unidade_id: form?.unidade_id || this.auth.unidade?.id,
    };

    if (form?.incluir_unidades_subordinadas) {
      filter.incluir_subordinadas = true;
    }
    if (form?.data_inicial) {
      filter.data_inicial = form.data_inicial.toISOString().slice(0, 10);
    }
    if (form?.data_final) {
      filter.data_final = form.data_final.toISOString().slice(0, 10);
    }
    if (form?.somente_vigentes) {
      filter.somente_vigentes = true;
    }
